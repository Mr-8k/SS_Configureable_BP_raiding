...
public class ShipWeaponsGroundRaidObjectivePluginImplMOD extends BaseGroundRaidObjectivePluginImpl {
    ...
    public ShipWeaponsGroundRaidObjectivePluginImplMOD(MarketAPI market) {
...
//		return Misc.getDGSCredits(value);
    ...
    public int performRaid(CargoAPI loot, Random random, float lootMult, TextPanelAPI text) {
        ...
        int minTieravailable = 0;
        boolean init = false;

        for (String id : faction.getKnownWeapons()) {
            WeaponSpecAPI spec = Global.getSettings().getWeaponSpec(id);
            if (init == false){
                minTieravailable = spec.getTier();
                init = true;
            }
            if (spec.getTier() < minTieravailable)
                minTieravailable = spec.getTier();
        }
        if (maxTier < minTieravailable)
            maxTier = minTieravailable;
